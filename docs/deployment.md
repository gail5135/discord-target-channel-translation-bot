...
이 문서는 봇을 GCP e2-micro 인스턴스에 올려 24시간 돌리는 절차다. 로컬 실행 방법은 `README_KR.md`를 본다(`README.md`는 언어 선택 페이지다).
...
| 공유 자원 | 확인할 것 |
|---|---|
| **RAM (약 1GB)** | 기존 앱의 `mem` 합계 + 이 봇의 약 45MB가 들어갈 여유가 있는지. 여유가 얇으면 4절의 스왑을 반드시 잡는다 |
...
**두 봇 모두 컴파일된 JS로 돌고 있다.** `pm2 ls`의 `mem` 실측:
...
합쳐 약 100MB다. RAM 약 1GB 안에서 여유가 있다.

**앞으로 이 인스턴스에 ts-node로 직접 실행하는 앱을 얹지 않는다.** ts-node는 TypeScript 컴파일러를 프로세스에 상주시켜 RSS를 약 290MB 더 쓴다(실측 369MB 대 80MB). 한 대만 얹어도 위 여유가 사라지고, 밀려나는 것이 새 앱이 아니라 **이미 돌던 봇**일 수 있다. 이 봇을 컴파일 실행으로 바꾼 이유가 그것이다(5절).

새 앱을 올리기 전에 무엇으로 도는지 확인한다:

```bash
pm2 describe <앱-이름> | grep -E "script path|script args"
```

`script path`가 `dist/...`면 컴파일 실행이고, `ts-node`나 `npm`이면 아니다.

**저장소의 `npm start`를 그대로 믿지 않는다.** `discord-forum-tracker`의 저장소는 `npm start`가 `ts-node src/index.ts`인데, 서버에는 빌드 후 `dist`를 실행하도록 등록되어 있다(위 51.9MB가 그 증거다 — ts-node였다면 300MB대다). 재등록하거나 다른 서버로 옮길 때 `npm start`를 쓰면 실행 방식이 조용히 바뀐다.
