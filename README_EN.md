...
**There are no credentials in this file.** API keys live in `.env`, and webhook tokens are **never saved to a file**. They are cached in process memory only, so they vanish when the bot restarts and are re-acquired per channel at that point.
