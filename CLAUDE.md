...
- `config.json`에는 **자격증명이 없다.** 번역 API 키는 `.env`에 있고, Webhook 토큰은 Phase 4에서 **디스크의 어느 파일에도 기록하지 않기로** 했다(프로세스 메모리에만 캐시하므로 재시작하면 사라지고, 그때 다시 조회한다). 담기는 것은 서버별 채널·언어 설정뿐이다. 그래도 공개할 정보는 아니므로 `.gitignore`와 파일 권한 `0o600`은 유지한다(쓰기 시 코드가 자동 적용)
