  ...
  apps: [
    {
      ...
      // 게이트웨이 연결과 길드 캐시를 얹어도 여유가 있다.
      //
      // 이 인스턴스는 이 봇 전용이 아니다. 기존에 운영 중이던 다른 디스코드 봇과
      // RAM 약 1GB를 나눠 쓰므로, 누수를 서버가 아니라 이 프로세스 선에서 끊어야
      // 옆 봇이 함께 밀려나지 않는다. 실제로 250M에 걸리는 일이 잦다면 누수이거나
      // 인스턴스 여유가 부족한 것이다 — docs/deployment.md의 문제 해결 표 참고.
